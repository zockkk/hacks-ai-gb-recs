...
export function Form() {
  ...
  const handleSubmit = useCallback(async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const response = await dataProcessApiService.process({
  ...
  const handleFileUpload = useCallback(async (file: File) => {
    ...
    const response: { recs: ResultItem[] } = await dataProcessApiService.upload(
      formData
    );
    setResult(response.recs);
  }, []);
        ...
        result.map((item, index) => (
          <div key={index} className="border p-4 rounded shadow">
            ...
            <div className="flex justify-between">
              <span>
                Косинусное расстояние по заголовку: {item.cosine_dist_title}
              </span>
              <span>
                Косинусное расстояние по описанию:{" "}
                {item.cosine_dist_description}
              </span>
            </div>
          </div>
        ))}
